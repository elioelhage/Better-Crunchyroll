...
const KEYS={shortcuts:'betterCrunchyrollShortcuts',videoQuality:'betterCrunchyrollVideoQuality',autoSkipIntro:'betterCrunchyrollAutoSkipIntro',autoSkipRecap:'betterCrunchyrollAutoSkipRecap',autoSkipCredits:'betterCrunchyrollAutoSkipCredits',blurUpcoming:'betterCrunchyrollBlurUpcoming',hideUpcomingTitles:'betterCrunchyrollHideUpcomingTitles'};
const DEFAULTS={shortcuts:{skip:'KeyS',previous:'KeyP',next:'KeyN'},videoQuality:'auto',autoSkipIntro:false,autoSkipRecap:false,autoSkipCredits:false,blurUpcoming:false,hideUpcomingTitles:false};
const QUALITY_VALUES=new Set(['auto','2160','1440','1080','720','480','360','240']);
...
function normalizeSettings(stored){const q=String(stored[KEYS.videoQuality]??DEFAULTS.videoQuality);return{shortcuts:{...DEFAULTS.shortcuts,...(stored[KEYS.shortcuts]||{})},videoQuality:QUALITY_VALUES.has(q)?q:DEFAULTS.videoQuality,autoSkipIntro:Boolean(stored[KEYS.autoSkipIntro]),autoSkipRecap:Boolean(stored[KEYS.autoSkipRecap]),autoSkipCredits:Boolean(stored[KEYS.autoSkipCredits]),blurUpcoming:Boolean(stored[KEYS.blurUpcoming]),hideUpcomingTitles:Boolean(stored[KEYS.hideUpcomingTitles])};}
...
function renderDraft(){for(const[id,button]of Object.entries(settingButtons))button.setAttribute('aria-checked',String(draftSettings[id]));for(const[key,label]of Object.entries(shortcutLabels))label.textContent=pretty(draftSettings.shortcuts[key]);videoQuality.value=draftSettings.videoQuality;confirmButton.disabled=equal(savedSettings,draftSettings);}
async function load(){const stored=await chrome.storage.local.get({[STORAGE_KEY]:true,[KEYS.shortcuts]:DEFAULTS.shortcuts,[KEYS.videoQuality]:DEFAULTS.videoQuality,[KEYS.autoSkipIntro]:false,[KEYS.autoSkipRecap]:false,[KEYS.autoSkipCredits]:false,[KEYS.blurUpcoming]:false,[KEYS.hideUpcomingTitles]:false});renderEnabled(stored[STORAGE_KEY]);savedSettings=normalizeSettings(stored);draftSettings=clone(savedSettings);renderDraft();versionText.textContent=`v${VERSION}`;popupElement.dataset.view=popupElement.dataset.view||'main';}
...
settingsButton.addEventListener('click',()=>{draftSettings=clone(savedSettings);activeShortcut=null;Object.values(shortcutButtons).forEach(b=>b.classList.remove('capturing'));mainHeader.hidden=true;mainView.hidden=true;settingsView.hidden=false;popupFooter.hidden=true;popupElement.dataset.view='settings';settingsView.scrollTop=0;renderDraft();});
...
videoQuality.addEventListener('change',()=>{draftSettings.videoQuality=QUALITY_VALUES.has(videoQuality.value)?videoQuality.value:DEFAULTS.videoQuality;renderDraft();});
